...
suite("AuthenticatedClient", () => {
  ...
  test(".getNotionalBalances() (with default currency)", async () => {
    const request = `/v1/notionalbalances/${DefaultCurrency}`;
    const account = "primary";
    const options = { request, account, nonce };
  ...
  test(".getNotionalBalances()", async () => {
    ...
    const data = await client.getNotionalBalances({ account, currency });
    deepStrictEqual(data, response);
  });
